...
# Aufruf:   docker build --tag juergenzimmermann/rentcar:2025.10.1-eclipse-noble .
#               ggf. --progress=plain
#               ggf. --no-cache
...
#           bash als Shell
#           docker save juergenzimmermann/rentcar:2025.10.1-eclipse-noble > rentcar.tar
...
FROM eclipse-temurin:${JAVA_VERSION}-jre-${UBUNTU_VERSION} AS final
...
LABEL org.opencontainers.image.title="rentcar" \
      org.opencontainers.image.description="Microservice rentcar v1 mit Basis-Image Eclipse Temurin und Ubuntu Noble" \
      org.opencontainers.image.version="2025.10.1-eclipse-noble" \
...
# https://manpages.debian.org/trixie/passwd/groupadd.html
# https://manpages.debian.org/trixie/adduser/addgroup.html
#groupadd --gid 1000 ubuntu
# https://manpages.debian.org/trixie/passwd/useradd.html
# https://manpages.debian.org/trixie/adduser/adduser.html
...
COPY --from=builder --chown=ubuntu:ubuntu ${BUILDER_APPLICATION_DIR}/lib/ ./lib/
COPY --from=builder --chown=ubuntu:ubuntu ${BUILDER_APPLICATION_DIR}/application.jar ./application.jar

# macOS auskommentieren:
# bei "Bind Mounts" (siehe compose.yml) werden die Zugriffsrechte fuer Linux nicht uebernommen
# d.h. trotz "chmod 777 ..." sind die Zugriffsrechte "755", also ohne Schreibrechte fuer group und others :-(
USER ubuntu:ubuntu
...
ENTRYPOINT ["java", "--enable-preview", "-classpath", "application.jar:lib/*.jar:.", "com.acme.rentcar.Application"]
