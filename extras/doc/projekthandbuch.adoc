...
== Der Microservice rentcar

=== Der Microservice im Überblick

Dieses Projekthandbuch dokumentiert den Microservice "rentcar", der für die Verwaltung von Fahrzeugen und deren Vermietung zuständig ist.

.Use Cases für den Microservice "rentcar"
[plantuml,use-cases,svg]
----
include::./use-cases.plantuml[]
----

Die Architektur basiert auf Spring Boot. Folgendes Diagramm zeigt die groben Komponenten. Test

.Komponenten des Microservice "rentcar"
...
Die Kommunikation nach außen erfolgt über eine REST-API. Der `CarController` stellt Endpunkte für CRUD-Operationen (Create, Read, Update, Delete) bereit.

."CarController" für die REST-Schnittstelle
[plantuml,CarController,svg]
----
include::./CarController.plantuml[]
----

=== Die Geschäftslogik

Die eigentliche Logik befindet sich in der Service-Schicht (`CarService`), die zwischen dem Controller und dem Repository vermittelt.

// Falls CarService.plantuml existiert, hier einkommentieren:
// .Die Service-Klasse "CarService" für die Geschäftslogik
// [plantuml,CarService,svg]
// ----
// include::./CarService.plantuml[]
// ----

=== Die Entity-Klassen

Die Datenhaltung erfolgt über JPA-Entities. Das folgende Diagramm zeigt die Struktur der `Car`-Entity und ihrer Abhängigkeiten.

.Die Entity-Klasse "Car"
[plantuml,Car,svg]
----
include::./Car.plantuml[]
----
